...
{
    public class EXoParser : BaseParser
    {
        public EXoParser()
        {
        ...
        public override RomSignatureObject Parse(string XMLFile, Dictionary<string, object>? options = null)
        {
            ...
            ParseHeader(signatureObject, xmlHeader, "eXo", md5Hash, sha1Hash);
            ...
            foreach (XmlNode xmlMachine in xmlMachines)
            {
                ...
                foreach (XmlNode childNode in xmlMachine.ChildNodes)
                {
                    switch (childNode.Name.ToLower())
                    {
                        ...
                        case "rom":
                            RomSignatureObject.Game.Rom rom = ParseRomAttributes(childNode, RomSignatureObject.Game.Rom.SignatureSourceType.eXo);
        ...
        public override parser.SignatureParser GetXmlType(XmlDocument xml)
        {
            ...
            XmlNode xmlName = xml.DocumentElement.SelectSingleNode("/datafile/header/name");

            if (xmlName == null || !xmlName.InnerText.Contains("eXoDOS"))
            {
                return parser.SignatureParser.Unknown;
            }

            XmlNodeList xmlMachine = xml.DocumentElement.SelectNodes("/datafile/game");
            ...
            return parser.SignatureParser.eXo;
        }
    }
}