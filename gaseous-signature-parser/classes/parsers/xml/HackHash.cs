...
{
    public class HackHashParser : BaseParser
    {
        public HackHashParser()
        {
        ...
        public override RomSignatureObject Parse(string XMLFile, Dictionary<string, object>? options = null)
        {
            ...
            ParseHeader(signatureObject, xmlHeader, "HackHash", md5Hash, sha1Hash);

            // parse platform
            string platformName = signatureObject.Name.Split('(').Last().TrimEnd(')');
            ...
            foreach (XmlNode xmlMachine in xmlMachines)
            {
                ...
                machineObject.System = platformName;
        ...
        public override parser.SignatureParser GetXmlType(XmlDocument xml)
        {
            ...
            XmlNode xmlAuthor = xml.DocumentElement.SelectSingleNode("/datafile/header/author");

            if (xmlAuthor == null || xmlAuthor.InnerText != "HackHash Community")
            {
                return parser.SignatureParser.Unknown;
            }

            return parser.SignatureParser.HackHash;
        }
    }
}